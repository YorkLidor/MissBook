...
const GOOGLE_URL = 'https://www.googleapis.com/books/v1/volumes?q=javascript%20power'
...
function query(txt) {
  return axios.get(GOOGLE_URL).then(books => {
    let newBooks = books.data.items.map(book => makeBookObj(book))
    if (txt) {
...
function makeBookObj(book) {
  const { title, subtitle, publishedDate, description, pageCount } = book.volumeInfo
  return {
    title,
    subtitle,
    publishedDate,
    ...
    listPrice: {
      amount: utilService.getRandomIntInclusive(10, 155),
      isOnSale: false
    }
  }
}
